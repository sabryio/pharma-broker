...
// GormReportRepo implements report data fetching using GORM
type GormReportRepo struct {
	db *GormDB
}

// NewGormReportRepo creates a new GORM-based ReportRepo
func NewGormReportRepo(db *GormDB) *GormReportRepo {
	return &GormReportRepo{db: db}
}

// GetMatchesForReport fetches matches with full contact details for reporting
// Uses raw SQL due to complex multi-table JOINs
func (r *GormReportRepo) GetMatchesForReport(ctx context.Context, config reports.ReportConfig) ([]*reports.MatchReport, error) {
	...
	err := r.db.DB.WithContext(ctx).Raw(query, periodStart, config.MinScore, limit).Scan(&results).Error
	if err != nil {
		return nil, err
	}
	...
	for i, res := range results {
		mr := &reports.MatchReport{
			...
			SellerPhone:       reports.FormatPhoneDisplay(res.SellerPhone),
			...
			BuyerPhone:        reports.FormatPhoneDisplay(res.BuyerPhone),
			BuyerGroupJID:     res.BuyerGroupJID,
			BuyerGroup:        res.BuyerGroupName,
			RequestCreatedAt:  res.RequestCreatedAt,
		}
		mr.SellerWhatsApp = reports.FormatWhatsAppLink(res.SellerPhone, res.MatchID)
		mr.BuyerWhatsApp = reports.FormatWhatsAppLink(res.BuyerPhone, res.MatchID)
		matchReports[i] = mr
	}
...
func (r *GormReportRepo) GetReportSummary(ctx context.Context, periodHours int) (*reports.ReportSummary, error) {
...
func (r *GormReportRepo) GetAlerts(ctx context.Context, periodHours int) ([]reports.Alert, error) {
	periodStart := time.Now().Add(-time.Duration(periodHours) * time.Hour)
	...
	// Low score matches pending review
