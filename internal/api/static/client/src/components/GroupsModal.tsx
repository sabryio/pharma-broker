  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          size="icon"
          className="fixed bottom-36 left-6 h-14 w-14 rounded-full shadow-lg z-50"
        >
      ...
      <DialogContent className="max-w-lg max-h-[80vh] flex flex-col" dir="rtl">
        ...
        <div className="border-b pb-3 mb-3">
          <Button onClick={handleSync} disabled={syncGroups.isPending}>
            <RefreshCw
              className={`ml-2 h-4 w-4 ${syncGroups.isPending ? 'animate-spin' : ''}`}
            />
            {syncGroups.isPending ? 'جاري المزامنة...' : 'مزامنة من واتساب'}
          </Button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-2">
          ...
          {!isLoading && (!groups || groups.length === 0) && (
            <p className="text-muted-foreground text-center py-8">
              لا توجد مجموعات. اضغط "مزامنة" لجلب المجموعات من واتساب.
            </p>
          )}
            ...
            >
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {group.message_count || 0} رسالة · آخر نشاط:{' '}
                  {group.last_message ? timeAgo(group.last_message) : 'لم يحدث'}
                </p>
              </div>
              <Switch
