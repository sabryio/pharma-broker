...
func TestWeightHistoryRepo_Save(t *testing.T) {
	...
	repo := NewWeightHistoryRepo(db.DB)
	...
	if current.Source != entity.WeightSourceManual {
...
func TestWeightHistoryRepo_GetCurrent(t *testing.T) {
	...
	histories := []*entity.WeightHistory{
		{
			ID:               uuid.New().String(),
			...
			PriceWeight:      0.10,
			RecencyWeight:    0.10,
			Source:           entity.WeightSourceDefault,
		...
		},
		{
			...
			PriceWeight:      0.15,
			RecencyWeight:    0.10,
			Source:           entity.WeightSourceAutoLearned,
			AppliedAt:        now,
		},
	}
	...
	if current.Source != entity.WeightSourceAutoLearned {
...
func TestWeightHistoryRepo_GetBySource(t *testing.T) {
	...
	// Get manual weights
	manualHistories, err := repo.GetBySource(ctx, entity.WeightSourceManual, 0)
	if err != nil {
	...
	for _, h := range manualHistories {
		if h.Source != entity.WeightSourceManual {
			t.Error("All histories should have MANUAL source")
		}
	}

	// Get auto-learned weights
	autoHistories, err := repo.GetBySource(ctx, entity.WeightSourceAutoLearned, 0)
	if err != nil {
		t.Fatalf("GetBySource failed: %v", err)
	}
...
func TestWeightHistoryRepo_SaveWithMetrics(t *testing.T) {
	...
	var savedMetrics entity.PerformanceMetrics
