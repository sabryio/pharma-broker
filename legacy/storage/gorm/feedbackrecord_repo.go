...
func NewFeedbackRecordRepo(db *DB) *FeedbackRecordRepo {
	return &FeedbackRecordRepo{db: db}
}

// Save stores a new feedback record
func (r *FeedbackRecordRepo) Save(ctx context.Context, feedback *entity.FeedbackRecord) error {
...
func (r *FeedbackRecordRepo) GetByID(ctx context.Context, id string) (*entity.FeedbackRecord, error) {
	...
	return ToFeedbackRecordEntity(&model), nil
}

// GetByDateRange retrieves feedback within a date range
func (r *FeedbackRecordRepo) GetByDateRange(ctx context.Context, startDate, endDate time.Time) ([]*entity.FeedbackRecord, error) {
	var models []FeedbackRecord
	err := r.db.Conn.WithContext(ctx).
	...
	return ToFeedbackRecordsEntity(models), nil
}

// GetFeedbackStats calculates aggregated statistics for a date range
func (r *FeedbackRecordRepo) GetFeedbackStats(ctx context.Context, startDate, endDate time.Time) (*entity.FeedbackStats, error) {
...
func (r *FeedbackRecordRepo) CountByAction(ctx context.Context, action entity.FeedbackAction, startDate, endDate time.Time) (int64, error) {
	var count int64
	err := r.db.Conn.WithContext(ctx).
		Model(&FeedbackRecord{}).
