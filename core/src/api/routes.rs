...
pub struct AppState<O, R, M, G, F, RQ>
where
    O: OfferRepository,
    R: RequestRepository,
    M: MatchRepository,
...
impl<O, R, M, G, F, RQ> Clone for AppState<O, R, M, G, F, RQ>
where
    O: OfferRepository,
    R: RequestRepository,
    M: MatchRepository,
...
pub fn create_router<O, R, M, G, F, RQ>(state: AppState<O, R, M, G, F, RQ>) -> Router
where
    ...
    RQ: ReviewQueueRepository + 'static,
{
    Router::new()
        ...
        .route(
            "/health/ready",
            get(handlers::health_ready::<O, R, M, G, F, RQ>),
        )
        .route("/health/live", get(handlers::health_live))
        // Prometheus metrics
        .route("/metrics", get(metrics_handler::<O, R, M, G, F, RQ>))
        // Offers
        .route(
            "/api/offers",
            get(handlers::get_offers::<O, R, M, G, F, RQ>),
        )
        // Requests
        .route(
            "/api/requests",
            get(handlers::get_requests::<O, R, M, G, F, RQ>),
        )
        // Matches
        .route(
            "/api/matches",
            get(handlers::get_matches::<O, R, M, G, F, RQ>),
        )
        .route(
            "/api/matches/{id}/confirm",
            post(handlers::confirm_match::<O, R, M, G, F, RQ>),
        )
        .route(
            "/api/matches/{id}/reject",
            post(handlers::reject_match::<O, R, M, G, F, RQ>),
        )
        // Stats
        .route("/api/stats", get(handlers::get_stats::<O, R, M, G, F, RQ>))
        // Groups (new CRUD endpoints)
        .route("/api/groups", get(groups::get_groups::<O, R, M, G, F, RQ>))
        .route(
            "/api/groups",
            post(groups::create_group::<O, R, M, G, F, RQ>),
        )
        .route(
            "/api/groups/{jid}",
            get(groups::get_group::<O, R, M, G, F, RQ>),
        )
        .route(
            "/api/groups/{jid}",
            put(groups::update_group::<O, R, M, G, F, RQ>),
        )
        .route(
            "/api/groups/{jid}",
            delete(groups::delete_group::<O, R, M, G, F, RQ>),
        )
        // Weights management
        .route(
            "/api/weights",
            get(weights::get_weights::<O, R, M, G, F, RQ>),
        )
        .route(
            "/api/weights",
            put(weights::update_weights::<O, R, M, G, F, RQ>),
        )
        .route(
            "/api/weights/scheduler",
            get(weights::get_scheduler_status::<O, R, M, G, F, RQ>),
        )
        .route(
            "/api/weights/influence",
            get(weights::get_influence::<O, R, M, G, F, RQ>),
        )
        .route(
            "/api/weights/abtest",
            get(weights::list_ab_tests::<O, R, M, G, F, RQ>),
        )
        .route(
            "/api/weights/abtest",
            post(weights::create_ab_test::<O, R, M, G, F, RQ>),
        )
        .route(
            "/api/weights/abtest/{id}",
            get(weights::get_ab_test_result::<O, R, M, G, F, RQ>),
        )
        .route(
            "/api/weights/abtest/{id}",
            delete(weights::end_ab_test::<O, R, M, G, F, RQ>),
        )
        // Review Queue
        .route(
            "/api/review-queue",
            get(review_queue::get_review_queue::<O, R, M, G, F, RQ>),
        )
        .route(
            "/api/review-queue/stats",
            get(review_queue::get_review_stats::<O, R, M, G, F, RQ>),
        )
        .route(
            "/api/review-queue/{id}",
            get(review_queue::get_review_item::<O, R, M, G, F, RQ>),
        )
        .route(
            "/api/review-queue/{id}/review",
            post(review_queue::update_review_status::<O, R, M, G, F, RQ>),
        )
        // WebSocket
        .route("/ws", get(ws::ws_handler::<O, R, M, G, F, RQ>))
        .with_state(state)
}

/// Handler for /metrics endpoint - returns Prometheus format
async fn metrics_handler<O, R, M, G, F, RQ>(
    axum::extract::State(state): axum::extract::State<AppState<O, R, M, G, F, RQ>>,
) -> impl IntoResponse
where
    O: OfferRepository,
    R: RequestRepository,
