...
function SnapshotCard({ snapshot, isActive }: { snapshot: MatchRecordingSnapshot; isActive: boolean }) {
    ...
    <div className={cn(
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className={cn('text-sm font-semibold', colors.text)}>{snapshot.event.label}</span>
            <p className="text-xs text-muted-foreground">{formatTime(snapshot.timestamp)}</p>
          </div>
        </div>
        <div className={cn(
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="p-3 rounded-xl bg-background/50 border border-border/30">
          <p className="text-[10px] text-muted-foreground uppercase tracking-wider font-semibold mb-1.5">Offer</p>
          <p className="text-sm font-medium text-foreground truncate">{snapshot.offer.product}</p>
        ...
        <div className="p-3 rounded-xl bg-background/50 border border-border/30">
          <p className="text-[10px] text-muted-foreground uppercase tracking-wider font-semibold mb-1.5">Request</p>
          <p className="text-sm font-medium text-foreground truncate">{snapshot.request.product}</p>
      ...
      {snapshot.aiStatus && (
        <div className="flex items-center gap-2 p-3 rounded-xl bg-violet-500/10 border border-violet-500/20 mb-4">
          ...
          <span className="text-sm text-violet-300">
            AI: {snapshot.aiStatus} ({snapshot.aiConfidence ? `${(snapshot.aiConfidence * 100).toFixed(0)}%` : 'N/A'})
          </span>
        </div>
      )}
      ...
      {snapshot.issues.length > 0 && (
        <div className="space-y-1.5 mb-4">
          {snapshot.issues.slice(0, 3).map((issue, idx) => (
            <div key={idx} className="text-xs text-amber-400 px-3 py-1.5 rounded-lg bg-amber-500/10 border border-amber-500/20">
              ⚠️ {issue}
            </div>
          ))}
          {snapshot.issues.length > 3 && (
            <p className="text-xs text-muted-foreground pl-1">+{snapshot.issues.length - 3} more issues</p>
          )}
        </div>
      )}
        ...
        <div className="pt-4 border-t border-border/30">
          ...
          <div className="grid grid-cols-3 gap-3 text-xs">
            <div className="p-2 rounded-lg bg-background/30 text-center">
              <span className="text-muted-foreground block mb-0.5">Medication</span>
              <span className="text-foreground font-semibold">{(snapshot.metadata.scoreBreakdown.medicationSimilarity * 100).toFixed(0)}%</span>
            </div>
            <div className="p-2 rounded-lg bg-background/30 text-center">
              <span className="text-muted-foreground block mb-0.5">Raw Text</span>
              <span className="text-foreground font-semibold">{(snapshot.metadata.scoreBreakdown.rawSimilarity * 100).toFixed(0)}%</span>
            </div>
            {snapshot.metadata.scoreBreakdown.embeddingSimilarity !== null && (
              <div className="p-2 rounded-lg bg-background/30 text-center">
                ...
                <span className="text-foreground font-semibold">{(snapshot.metadata.scoreBreakdown.embeddingSimilarity * 100).toFixed(0)}%</span>
              </div>
            )}
          </div>
        </div>
          ...
          return (
            ...
            >
              ...
              <div className={cn(
                ...
                <p className="text-muted-foreground">{formatTime(snapshot.timestamp)}</p>
              </div>
...
export function RecordingPlayback({ recording, onClose, onExport, onDelete }: RecordingPlaybackProps) {
  const [currentIndex, setCurrentIndex] = useState(recording.snapshots.length - 1)
  ...
  const goToEnd = useCallback(() => setCurrentIndex(totalSnapshots - 1), [totalSnapshots])
  const goToPrev = useCallback(() => setCurrentIndex((i) => Math.max(0, i - 1)), [])
  const goToNext = useCallback(() => setCurrentIndex((i) => Math.min(totalSnapshots - 1, i + 1)), [totalSnapshots])
    ...
    <div className={cn(
      ...
      <div className="flex items-center justify-between p-4 border-b border-border/50 bg-gradient-to-r from-slate-900/80 to-slate-800/80">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-foreground">Recording Playback</h3>
            <p className="text-xs text-muted-foreground">
              Match <span className="font-mono">#{recording.matchId.slice(0, 8)}</span> • {totalSnapshots} snapshots
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-secondary/50 text-xs text-muted-foreground">
            ...
            {recording.duration ? formatDuration(recording.duration) : 'In progress'}
          </div>
      ...
      <div className="px-6 py-4 border-b border-border/30 bg-secondary/20">
        <Timeline snapshots={recording.snapshots} currentIndex={currentIndex} onSelect={setCurrentIndex} />
      </div>

      {/* Playback Controls */}
      <div className="flex items-center justify-center gap-3 p-4 border-b border-border/30 bg-secondary/10">
        <button onClick={goToStart} className="p-2 rounded-lg bg-secondary/50 hover:bg-secondary text-muted-foreground hover:text-foreground transition-colors" title="Go to start">
          <SkipBack className="w-4 h-4" />
        </button>
        <button onClick={goToPrev} disabled={currentIndex === 0} className="p-2 rounded-lg bg-secondary/50 hover:bg-secondary text-muted-foreground hover:text-foreground transition-colors disabled:opacity-30" title="Previous">
        ...
        <button onClick={goToNext} disabled={currentIndex === totalSnapshots - 1} className="p-2 rounded-lg bg-secondary/50 hover:bg-secondary text-muted-foreground hover:text-foreground transition-colors disabled:opacity-30" title="Next">
          <ChevronRight className="w-5 h-5" />
        </button>
        <button onClick={goToEnd} className="p-2 rounded-lg bg-secondary/50 hover:bg-secondary text-muted-foreground hover:text-foreground transition-colors" title="Go to end">
      ...
      {/* Snapshot Content */}
      <div className={cn('p-6 overflow-auto', isExpanded ? 'max-h-[calc(100vh-280px)]' : 'max-h-[400px]')}>
