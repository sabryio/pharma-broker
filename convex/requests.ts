...
export const getById = query({
  ...
  handler: async (ctx, args) => {
    try {
      ...
      const docId = ctx.db.normalizeId("requests", args.id);
...
export const updateStatus = mutation({
  ...
  handler: async (ctx, args) => {
    const docId = args.id as Id<"requests">;
...
// Search requests by medication (LIKE %query% approximate)
...
// Request with score
type RequestWithScore = Doc<"requests"> & { _score: number };
...
export const searchSimilar = action({
  ...
  handler: async (ctx, args): Promise<RequestWithScore[]> => {
    ...
    const requestsWithScores: RequestWithScore[] = [];
    ...
    for (const result of results) {
      ...
      if (request) {
        if (
          (!args.withinMs || request.createdAt >= cutoff) &&
          (!args.similarityThreshold ||
            result._score >= args.similarityThreshold)
        ) {
          requestsWithScores.push({ ...request, _score: result._score });
        }
      }
    }
...
// Delete requests created before cutoff (matches SeaORM delete_before)
